...
import { viaEngagementScope } from "@/lib/ownership";
...
export async function bulkDeleteFindings(formData: FormData) {
  ...
  if (ids.length) await prisma.finding.deleteMany({ where: { id: { in: ids }, ...viaEngagementScope(email) } });
  revalidatePath("/dashboard/findings");
}
...
export async function bulkSetStatus(formData: FormData) {
  ...
  if (ids.length && STATUSES.includes(status)) {
    await prisma.finding.updateMany({ where: { id: { in: ids }, ...viaEngagementScope(email) }, data: { status } });
...
export async function bulkSetCategory(formData: FormData) {
  ...
  if (ids.length && category) {
    await prisma.finding.updateMany({ where: { id: { in: ids }, ...viaEngagementScope(email) }, data: { category } });
  }
  revalidatePath("/dashboard/findings");
}
