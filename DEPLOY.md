...
|---|---|---|
| `DATABASE_URL` | ✅ | Postgres connection string (Neon: the pooled `-pooler` host is fine) |
| `DIRECT_URL` | optional | Direct (non-pooled) connection for migrations — Neon: `DATABASE_URL` with `-pooler` removed. If unset, the build falls back to `DATABASE_URL`. Set it only if `prisma migrate deploy` hits a P1002 advisory-lock timeout. |
